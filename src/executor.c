...
cstr *execute_ast(AstNode *node) {
    ...
    if (is_op_token(node_value)) {
        ...
        {
            case '*':
				return token_mlt(left_node_value, execute_ast(right_node));
                break;
            case '/':
                return token_div(left_node_value, execute_ast(right_node));
                break;
			case '+':
                return token_add(left_node_value, execute_ast(right_node));
        		break;
			case '-':
                return token_sub(left_node_value, execute_ast(right_node));
            	break;
